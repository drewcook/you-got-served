	...
	<Layout title="Table Details">
		<Link href="/Tables" as="/tables">
			<button className="btn btn-secondary">
				<i className="fas fa-angle-left"></i> Back To Tables
			</button>
		</Link>
		<h2>Table Details</h2>
		...
		<Query query={GET_TABLE_BY_ID} variables={{ id: props.router.query.id }}>
			{({ data: { getTableById }, loading, error }) => {
				...
				return (
					<div>
						<h5>
							Number: <TableNumber number={table.number} />
						</h5>
						{/*<h5>ID: <em>{table.id}</em></h5> not needed for a user */}
						<Mutation
							mutation={ADD_CHECK}
							variables={{ tableId: props.router.query.id }}
							refetchQueries={[
								{
									query: GET_CHECKS_FOR_TABLE,
									variables: { tableId: props.router.query.id },
								},
							]}
						>
								...
								return (
									<div>
										...
										<button
											...
											disabled={loading}
										>
											Add Check <i className="fas fa-plus"></i>
										</button>
										{error && (
											<p className="text-danger">
												<small>
													You may not add another check if one is already open.
												</small>
											</p>
										)}
									</div>
