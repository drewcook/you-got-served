		...
		return (
			<Layout title="Check Details">
				...
				<Query
					...
					variables={{ checkId: this.props.router.query.id }}
				>
					{({ data: { getCheckDetails }, loading, error }) => {
						...
						return (
							<div>
								<Link
									href={`/table?id=${details.tableId}`}
									as={`/table/${details.tableId}`}
								>
									<button className="btn btn-secondary">
										<i className="fas fa-angle-left"></i> Table Details
									</button>
								</Link>
								{/*<p>Check ID: {details.id}</p> not needed for a user */}
								<Query
									query={GET_TABLE_BY_ID}
									variables={{ id: details.tableId }}
								>
								...
								<p className="text-muted">
									Opened on{" "}
									{moment(details.dateCreated).format("MM/DD/YYYY @ h:mm A")}
								</p>
								<p className="text-muted">
									Last Modified on{" "}
									{moment(details.dateUpdated).format("MM/DD/YYYY @ h:mm A")}
								</p>
								...
								<div className="items-wrapper border-primary">
									<h4 className="d-flex align-content-center justify-content-between">
										...
										{!details.closed && (
											<div>
												{details.orderedItems.filter((item) => !item.voided)
													.length > 0 && (
													<button
														className="btn btn-sm btn-outline-danger"
														onClick={this.openVoidModal}
													>
														Void Item <i className="fas fa-ban"></i>
													</button>
												)}
												<button
									...
									<hr />
									{details.orderedItems.length ? (
										details.orderedItems.map((item, idx) => (
											<div
												className={
													item.voided
														? "line-item voided text-muted"
														: "line-item"
												}
												key={idx}
											>
										...
										<p className="text-center">
											<em>There are currently no items on this check.</em>
										</p>
									)}
									{details.closed && (
										<React.Fragment>
											...
											<div className="line-item">
												Tax <span>${details.tax}</span>
											</div>
											<div className="line-item">
												Tip <span>${Math.round(details.tip * 100) / 100}</span>
											</div>
										</React.Fragment>
									)}
									<hr />
											...
											{details.orderedItems
												...
												.map((item, idx) => (
													<ListGroup.Item
														key={idx}
														action
														onClick={() => this.selectVoidItem(item.id)}
														style={{ outline: "none" }}
														className="d-flex align-content-center justify-content-between"
													>
														{item.name}
									...
									<Modal.Footer>
										...
										>
											{(voidItem) => (
												<button
													className="btn btn-sm btn-success"
													onClick={() => this.voidItem(voidItem)}
													disabled={this.state.voidedItem === null}
												>
													Void Item
												</button>
											)}
								...
								{!details.closed && (
									<Mutation
										mutation={CLOSE_CHECK}
										variables={{ checkId: details.id }}
										refetchQueries={[
											{
												query: GET_CHECK_DETAILS,
												variables: { checkId: this.props.router.query.id },
									...
									>
										{(closeCheck) => (
											<button className="btn btn-danger" onClick={closeCheck}>
												Close Check
											</button>
										)}
									</Mutation>
